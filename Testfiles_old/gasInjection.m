...
c   = (47:1:53).';
src = addSource([], c, 0.01*ones(size(c))*kilogram ./ day(), 'sat', [0,1]);
display(src);

%bc = fluxside([], G, 'xmin', injRate, 'sat', [1, 0]);
bc = pside([], G, 'xmin', 1*barsa, 'sat', [1,0]);
...
schedule = simpleSchedule(repmat(dT,1,n), 'bc', bc,'src',src);
