function [BCTocellMap, BCcells, qRes] = getBoundaryConditionFluxes(G, pressure, rho, mob, T,  s,  bc)
...
totMob = double(zeros(nbc, 1));
for i = 1:nPh
    totMob = totMob + cellToBCMap*mob(:,i);
end

[q_s, qRes] = deal(double(zeros(nbc, 2)));
for i = 1:nPh
    q_r = deal(double(zeros(nbc, 1)));
