function q = addfluxfromSourceandBC(state, G,  src, bc, varargin)
%Compute source term contributions for transport
%
% SYNOPSIS:
%   q = addfluxformSourceandBC(state, G, wells, src, bc)
...
%       boundary conditions or synthetic fluid sources (i.e., the
%       'src' parameter).
%
...
function check_input(bc, src, opt)
   if opt.use_compi,       
